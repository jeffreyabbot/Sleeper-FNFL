...
@st.cache_data(ttl=86400)  # Cache for 24 hours to prevent unnecessary daily downloads
...
try:
    ...
    # Calculate fantasy points on raw weekly if not already calculated
...
with tab_leaderboard:
    ...
    with col1:
        ...
        filtered_averages = df_agg_averages[df_agg_averages['FantasyOwner'].isin(owner_filter)]
        
        st.markdown("**Top Performers Table**")
        
        filtered_totals_styled = (
            filtered_totals[['PlayerName', 'Team', 'FantasyPoints', 'FantasyOwner']]
            .rename(columns={'FantasyPoints': 'Total Points'})
    ...
    with col2:
        ...
        # Interactive Search / Highlight Box
        ...
        # Adjust dots based on the highlighted player selection
...
with tab_trends:
    ...
    if selected_players:
        df_trends = df_raw_weekly[df_raw_weekly['PlayerName'].isin(selected_players)]
        ...
        st.warning("Please select at least one player to view the trend line.")
