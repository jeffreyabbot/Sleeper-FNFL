    ...
    if not roster_map:
        ...
        if df_teams_raw.empty:
            ...
            with col_tbl:
                ...
                # Format to 2 decimal places and apply a beautiful Purple heatmap gradient
            ...
            with col_rad:
                st.markdown("**Positional Strength Spider Chart Comparison**")
                
                # Multi-selector to pick which teams to plot on the radar (spider) chart
                selected_teams = st.multiselect(
                    "Select Teams to Compare:",
                    options=list(df_teams_with_avg['Owner'].unique()),
                ...
                if selected_teams:
                    ...
                    df_radar = df_teams_with_avg[df_teams_with_avg['Owner'].isin(selected_teams)].copy()
                    df_radar_melted = df_radar.melt(
                        id_vars=['Owner'],
                        value_vars=available_positions,
                        var_name='Position',
                        value_name='Total Points'
                    )
                    
                    # Create the interactive Radar (Spider) Chart
                    fig_radar = px.line_polar(
                        ...
                        color='Owner',
                        line_close=True,
                        title="Team Positional Strength comparison vs. League Average"
                    )
            ...
            # --- NEW ADDITION: TEAM ROSTER EXPLORER SECTION ---
